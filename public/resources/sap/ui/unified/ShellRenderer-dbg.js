...
sap.ui.unified.ShellRenderer.renderSearch = function(rm, oShell) {
	...
	rm.write("</div>");
	sap.ui.unified.ShellRenderer._renderIcon(rm, oShell, true, bShowSearch ? "sapUiUfdShellHidden" : "");
};
...
sap.ui.unified.ShellRenderer.renderHeaderItems = function(rm, oShell, begin) {
	...
	rm.write("</div>");
	if(begin){
		sap.ui.unified.ShellRenderer._renderIcon(rm, oShell, false, "");
	}
};

sap.ui.unified.ShellRenderer._renderIcon = function(rm, oShell, bCenter, sStyleClass) {
	var rb = sap.ui.getCore().getLibraryResourceBundle("sap.ui.unified"),
		sLogoTooltip = rb.getText("SHELL_LOGO_TOOLTIP"),
		sIco = oShell._getIcon();
	
	rm.write("<div class='sapUiUfdShellIco "+sStyleClass+"'>");
	rm.write("<img id='", oShell.getId(), bCenter ? "-icon-center'" : "-icon'");
